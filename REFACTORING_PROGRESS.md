...
- [x] Move all `ViewModel` classes to a unified `ui/vm` package.
- [x] Relocate `MainActivity.kt` and `MainApplication.kt` to a more appropriate package (e.g., `ui` or `app`).
- [x] Create a dedicated `model` package for all data classes (e.g., `Settings.kt`, `Limiter.kt`).
...
- [x] Introduce a dependency injection framework (e.g., Hilt) to manage dependencies and improve testability.
